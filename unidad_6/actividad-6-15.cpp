...
p_escondida *cargarPartida(char *nombreArchivo);
void salvarPartida(char *nombreArchivo, p_escondida *escondidas);
...
int main(int argc, char const *argv[]){
  ...
  char nombreArchivoDatos[] = "data";
  p_escondida *escondidas;

  escondidas = cargarPartida(nombreArchivoDatos);

  char nombreArchivo[] = "palabras.txt";
  if(escondidas != NULL){
    cout << "¿Quiere retomar (r) o empezar de nuevo (n)?\n";
    tecla = getch();
    if(tecla == 'n'){
      char **palabras = cargarPalabras(nombreArchivo, totalPalabras);
      escondidas = acomodarPalabras(palabras, totalPalabras);
    }
  } else {
    char **palabras = cargarPalabras(nombreArchivo, totalPalabras);
    escondidas = acomodarPalabras(palabras, totalPalabras);
  ...
  salvarPartida(nombreArchivoDatos, escondidas);
...
p_escondida *cargarPartida(char *nombreArchivo){
  ...
  if(archivo.good()){
    for (int i = 0; i < dificultad; ++i){
      archivo.getline(escondidas[i].letras, 21, ';');
      archivo.getline(temp, 3, ';');
      escondidas[i].x = atoi(temp);
      archivo.getline(temp, 3, ';');
      escondidas[i].y = atoi(temp);
      archivo.getline(temp, 2, ';');
      escondidas[i].vertical = atoi(temp);
      archivo.getline(temp, 2, '#');
      escondidas[i].encontrada = atoi(temp);
    }
  } else {
    return NULL;
  }
...
void salvarPartida(char *nombreArchivo, p_escondida *escondidas){
  ...
  for (int i = 0; i < dificultad; ++i){
    archivo << escondidas[i].letras 
      << ';' << escondidas[i].x 
      << ';' << escondidas[i].y 
      << ';' << escondidas[i].vertical 
      << ';' << escondidas[i].encontrada 
      << '#';
  }
